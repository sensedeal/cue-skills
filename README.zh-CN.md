...
| [`cue-omni-reader/`](./cue-omni-reader) — **cue-omni-reader** | 帮外部 agent 通过官方 MCP 工具解析 HTTP(S) URL 与已授权的本地文档、音频、视频，覆盖安全 Bridge 自举、可恢复 operation、仅传统文本客户端兼容、完整 artifact 读取与清理。薄指令层，不含自定义解析器或协议 driver。 | v0.3.2 |
