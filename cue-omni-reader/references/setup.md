...
- **Audited package:** `@cueai/omni-reader-mcp@1.8.3`
...
```sh
npx -y @cueai/omni-reader-mcp@1.8.3 setup
```
...
Current audited release: **Bridge 1.8.3** — pin-sync release: the trusted managed-entry pair advances to **1.8.2 / 1.8.3** and the data-plane error codes are renamed to the package's public identity (`OMNI_READER_*`, `INSECURE_OMNI_READER_URL`; hard cut, no alias). No tool-surface or behavior change from 1.8.2, which brought remote URL `grounded`/`layout` end to end (capability negotiation, local `kind: bundle` install, and cross-process bundle recovery; live since 2026-09-23). The only bare-`npx` Windows migration exception remains 1.5.1. See the [1.8.3 publication report](../docs/verification-reports/2026-09-23-bridge-1.8.3-published.md); per-release history is in [`compatibility.md`](./compatibility.md).


Supported non-interactive native-adapter examples:

```sh
npx -y @cueai/omni-reader-mcp@1.8.3 setup --client hermes --allowed-root /absolute/minimum/root --yes --json
npx -y @cueai/omni-reader-mcp@1.8.3 setup --client cursor --add-root /absolute/minimum/root --yes --json
npx -y @cueai/omni-reader-mcp@1.8.3 setup --client claude-desktop --allowed-root /absolute/minimum/root --yes --json
```
...
```sh
npx -y @cueai/omni-reader-mcp@1.8.3 doctor --json
```
...
```sh
npx -y @cueai/omni-reader-mcp@1.8.3 doctor --json --silent-check
```
