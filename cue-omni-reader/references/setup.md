...
Bridge 1.5.5 keeps the seven-tool surface and advances the trusted predecessor. Bridge 1.6.0 keeps those seven names, adds explicit `result_delivery="artifact"`, direct outline-before-save navigation, monotonic same-source `auto`→`artifact` reuse, signed chunk cursors, bounded explicit config inspection, exact MSYS path normalization, and journal v4 keyed identities. The npm registry tarball was verified byte-identical to the reviewed package, and one separately authorized production acceptance identity completed with navigation leaving the operation journal unchanged. Bridge 1.7.0 keeps the same seven tools and adds operation-local billing transparency for the unified URL/local result experience. Bridge 1.7.1 keeps that contract, corrects `BRIDGE_UPGRADE_REQUIRED` guidance so an already-latest client does not reinstall or retry, and advances the exact trusted pair to 1.7.0/1.7.1 while retaining only the Windows bare-`npx` 1.5.1 migration exception. Bridge 1.7.2 keeps the same seven-tool surface and wire contract — it is a client-side compatibility release, not a protocol change: the local extension allowlist that gates a source file before it is even sent (independent of what the server can parse) now also accepts archive formats `rar`/`tar`/`tgz`/`gz`/`bz2` and plain-text formats `tsv`/`json`/`yaml`/`yml`/`toml`/`xml`/`ini`/`cfg`/`conf`/`log` plus the RDF family and FreeMind, all of which the server already parsed by content-sniffing — the allowlist had simply lagged behind server capability. `parquet` and mind-map container formats (`xmind`/`mmap`) remain deliberately unsupported. The trusted pair advances to 1.7.1/1.7.2. Native WorkBuddy skill loading and its four client capabilities remain unverified; see the current [publication report](../docs/verification-reports/2026-09-08-bridge-1.7.3-published.md). Bridge 1.7.3 keeps the same seven-tool surface and is a metadata-only release: the package `license` field is corrected from `UNLICENSED` to `MIT` and the version advances `1.7.2 → 1.7.3` (npm will not republish an already-existing version with changed content, so the license fix ships as a new patch release). There is no tool-surface or parsing change; the extension allowlist and the seven tools remain exactly as 1.7.2. The trusted managed-entry pair advances to 1.7.2/1.7.3. Bridge 1.7.3 is published and `latest` on npm; cue-skill publication remains owner-gated.
