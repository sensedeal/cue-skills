...
- **Audited package:** `@cueai/omni-reader-mcp@1.8.2`
...
```sh
npx -y @cueai/omni-reader-mcp@1.8.2 setup
```
...
Current audited release: **Bridge 1.8.2** — remote URL `grounded`/`layout` now works end to end: the Bridge negotiates the service's advertised reader capabilities before a non-text `parse` (1.8.1 was rejected at that step), and installs the returned result bundle locally as `kind: bundle`; a completed bundle is also recovered from the local cache by a later Bridge process instead of being refetched. Requires the Omni service that advertises the URL grounding profile (live since 2026-09-23). The trusted managed-entry pair is **1.8.1 / 1.8.2**; the only bare-`npx` Windows migration exception remains 1.5.1. See the [1.8.2 publication report](../docs/verification-reports/2026-09-23-bridge-1.8.2-published.md); per-release history is in [`compatibility.md`](./compatibility.md).


Supported non-interactive native-adapter examples:

```sh
npx -y @cueai/omni-reader-mcp@1.8.2 setup --client hermes --allowed-root /absolute/minimum/root --yes --json
npx -y @cueai/omni-reader-mcp@1.8.2 setup --client cursor --add-root /absolute/minimum/root --yes --json
npx -y @cueai/omni-reader-mcp@1.8.2 setup --client claude-desktop --allowed-root /absolute/minimum/root --yes --json
```
...
```sh
npx -y @cueai/omni-reader-mcp@1.8.2 doctor --json
```
...
```sh
npx -y @cueai/omni-reader-mcp@1.8.2 doctor --json --silent-check
```
