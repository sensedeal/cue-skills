...
Current: **Bridge 1.8.3** — data-plane error codes renamed to the package's public identity (`OMNI_READER_UNAVAILABLE` / `OMNI_READER_INVALID_RESPONSE` / `OMNI_READER_PARSE_FAILED` / `INSECURE_OMNI_READER_URL`). The rename is a hard cut with no alias; automation branching on the retired codes must match the new names. `retryable` / `user_action` per code are unchanged. No tool-surface change. The trusted managed-entry pair advances to **1.8.2 / 1.8.3**; the only bare-`npx` Windows migration exception remains 1.5.1. See the [1.8.3 publication report](../docs/verification-reports/2026-09-23-bridge-1.8.3-published.md).

Previous (1.8.2): **Bridge 1.8.2** — remote URL `grounded`/`layout` is live. The Bridge sends `initialize` before a non-text `parse` and uses only `capabilities.experimental["cue.omni-reader"]` (absent → `DETAIL_CAPABILITIES_UNAVAILABLE`, never inferred); 1.8.1 sent that request with a single `Accept` value and every remote non-text parse was rejected there (no operation, not billed). The service (cube-mcp 1.5.52, 2026-09-23) labels a completed bundle with `media_type=application/vnd.cue.omni-result-bundle+json; version=1`; the Bridge installs it as a local `kind: bundle` whose content part is the verified Markdown. A completed bundle is now recovered from the local cache by a later Bridge process (1.8.1 refetched an already-delivered result and got a retryable error). `detail=text` and omitted `detail` are unchanged. No tool-surface change. The trusted managed-entry pair advanced to **1.8.1 / 1.8.2**; the only bare-`npx` Windows migration exception remains 1.5.1. See the [1.8.2 publication report](../docs/verification-reports/2026-09-23-bridge-1.8.2-published.md).
...
|---|---|---|---|
| 1.8.3 | 2026-09-23 | data-plane error codes renamed to `OMNI_READER_*` / `INSECURE_OMNI_READER_URL` (hard cut, no alias); pin sync only — no behavior change | [report](../docs/verification-reports/2026-09-23-bridge-1.8.3-published.md) |
| 1.8.2 | 2026-09-23 | remote URL `grounded`/`layout` live: capability negotiation accepts JSON and SSE; completed bundles recover locally across processes | [report](../docs/verification-reports/2026-09-23-bridge-1.8.2-published.md) |
...
- **`detail` on the remote**: the remote `parse` accepts the Bridge's `detail`. `text` (or omitted) forwards unchanged; `grounded`/`layout` fails closed with `UNSUPPORTED_DETAIL` (`failure_scope=local_capability`) before any operation is created — the remote produces markdown output only and never silently downgrades a requested representation. *(Superseded 2026-09-23: with cube-mcp 1.5.52 and Bridge 1.8.2 the remote serves `grounded`/`layout` as a result bundle; it still never downgrades, and fails closed with `UNSUPPORTED_DETAIL` wherever the URL grounding profile is unavailable.)*
