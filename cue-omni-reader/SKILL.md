...
2. **Use an available official Omni tool first.** A URL does not require local Bridge installation when a suitable remote Omni tool is connected. A local source requires the official Bridge and an allowed root.
