...
2. **`npx` shell form** — `npx -y @cueai/omni-reader-mcp@1.8.1` from a shell that resolves `.cmd`
...
Run `npx -y @cueai/omni-reader-mcp@1.8.1 doctor --json` first, then diagnose by structured error code and keep the control-plane and upload stages separate:
