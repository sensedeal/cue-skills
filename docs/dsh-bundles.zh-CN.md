...
两者都是 `@cueai/*`(公开 scope)、MIT,并在英文 README 旁提供 `README.zh-CN.md`。

## 分发与校验

- **安装**:`dsh plugin --profile web add @cueai/dsh-omni-reader`(+ 加固层 `…-guard`);配置与 FAQ 见各 bundle 的 `README.md` 与 [`dsh/usage.md`](../dsh/usage.md)。
...
wiring bundle 负责接入 server;guard bundle(可选)负责限制 SSRF 与费用。两者一起,让 Harness profile 把 Cue Omni Reader 暴露成原生 `mcp__omni__*` 工具,并带一层 fail-closed 的安全护栏。
