...
|---|---|---|
| [`cue-buddy/`](./cue-buddy) — **cue-buddy** | Lets business experts author, validate, test, tune, and pin-as-frequent [Cue](https://cuecue.cn) "buddy" research templates via natural conversation. No Python / API knowledge needed; the agent talks to the Cue production API on the user's behalf. | v0.2.2 |
| [`cue-research/`](./cue-research) — **cue-research** | Sibling to cue-buddy for *using* Cue from inside your AI agent: ask a question → skill matches ≤2 candidate buddies (or routes to free-form deep research via `/api/rewrite`) → you confirm credits → it runs in the background and retrieves the report via replay → on satisfaction you can distill a free-form run into a saved buddy via cue-buddy. Supports 仿写/mimic (imitate a reference URL or sample document's writing style). | v0.3.3 |
